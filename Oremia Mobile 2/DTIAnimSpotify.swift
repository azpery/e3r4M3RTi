...
class DTIAnimSpotify: DTIAnimProtocol {
    ...
    fileprivate let animationDuration = CFTimeInterval(1.2)
    fileprivate let circleCount = 3
    ...
    func needLayoutSubviews() {
        ...
        let circleWidth: CGFloat = contentSize.width / CGFloat(circleCount*10+1)
        let circlePos: CGFloat = contentSize.width / CGFloat(circleCount*2+1)
        let posY: CGFloat = (contentSize.height-circlePos)/2
        ...
        for index in (0 ..< circleCount) {
            ...
            circleLayer.frame = CGRect(x: ((contentSize.width/2)-(2*circleWidth))+CGFloat(index)*(circleWidth*2), y: posY, width: circleWidth, height: circleWidth)
