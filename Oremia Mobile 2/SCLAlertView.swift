...
public class SCLAlertView: UIViewController {
    ...
    public func showWarning(title: String, subTitle: String, closeButtonTitle:String?=nil, duration:NSTimeInterval=0.0, colorStyle: UInt=0x727375, colorTextButton: UInt=0xFFFFFF) -> SCLAlertViewResponder {
    ...
    public func showTitle(title: String, subTitle: String, duration: NSTimeInterval?, completeText: String?, style: SCLAlertViewStyle, colorStyle: UInt?, colorTextButton: UInt?) -> SCLAlertViewResponder {
        ...
        if !subTitle.isEmpty {
                ...
                let attr = [NSFontAttributeName:self.viewText.font ?? UIFont()]
                let sz = CGSize(width: self.kWindowWidth - 24, height:90)
                let r = str.boundingRectWithSize(sz, options: NSStringDrawingOptions.UsesLineFragmentOrigin, attributes:attr, context:nil)
                let ht = ceil(r.size.height)
                if ht < self.kTextHeight {
                self.kWindowHeight -= (self.kTextHeight - ht)
                self.kTextHeight = ht
                }
        }
