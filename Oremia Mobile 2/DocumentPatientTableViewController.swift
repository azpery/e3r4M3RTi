...
class DocumentPatientTableViewController: UITableViewController, APIControllerProtocol, QLPreviewControllerDataSource,UIPopoverPresentationControllerDelegate {
    ...
    @IBAction func performSigningAction(sender: AnyObject) {
        if self.signing {
            self.signing = false
    ...
    override func numberOfSectionsInTableView(tableView: UITableView) -> Int {
        var i = 0
        if (lesDocuments.count > 0 ){i++;self.signerButton.enabled = true}else{self.signerButton.enabled = false}
        if (lesModeleDocuments.count > 0 && !self.signing){i++}
        if (lesDocumentsWord.count > 0 && !self.signing){i++}
    ...
    override func tableView(tableView: UITableView, commitEditingStyle editingStyle: UITableViewCellEditingStyle, forRowAtIndexPath indexPath: NSIndexPath) {
        if editingStyle == UITableViewCellEditingStyle.Delete {
            ...
            if(indexPath.section == 0 && lesDocuments.count>0){
                ...
                self.lesDocuments.removeAtIndex(row)
                ...
                self.lesModeleDocuments.removeAtIndex(row)
            }else{
                idDoc = lesDocumentsWord[row].id
                query = "DELETE FROM documents WHERE id = \(idDoc)"
                self.lesDocumentsWord.removeAtIndex(row)
            }
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        ...
        }else if segue.destinationViewController.isKindOfClass(UINavigationController) && segue.identifier == "showSignature"{
    ...
    func insertSignature(sPrat:String, sPatient:String) -> Void{
        ...
        api?.signDocument(sPrat, sPatient: sPatient, idDoc: self.selectedDoc, idPatient: self.patient!.id , success: {id -> Bool in
            ...
            self.lesDocuments.append(Document(id: id, nom: self.lesDocuments[self.documentTableView.indexPathForSelectedRow!.row].nom, date: ToolBox.getFormatedDate(NSDate())))
